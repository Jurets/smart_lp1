...
//    public $log_reff;
    ...
    protected function setClubUsers() {
        ...
        $club_all_SQL = "SELECT id, username, tariff_id, club_date, purse
                    FROM tbl_users
                    WHERE tariff_id IN(3, 4, 5, 6)";
        $source_сlub_all = $this->dbh->createCommand($club_all_SQL)->query()->readAll();
        if (is_null($source_сlub_all)) {
            throw new CHttpException('500', 'No Club Users present');
        }
        ...
        foreach ($source_club_all as $club) {
            switch ($club['tariff_id']) {
                case '3':
                    ...
                    $this->club_users['B0']['struct'][$b0]['username'] = $club['username'];
                    $this->club_users['B0']['struct'][$b0]['purse'] = $club['purse'];
                    $b0 ++;

                    /* Счетчик обновляется только здесь */
                ...
                case '4':
                    ...
                    $this->club_users['B1']['struct'][$b1]['username'] = $club['username'];
                ...
                case '5':
                    ...
                    $this->club_users['B2']['struct'][$b2]['username'] = $club['username'];
                ...
                case '6':
                    ...
                    $this->club_users['B3']['struct'][$b3]['username'] = $club['username'];
...
//                AND tr_kind_id = 5 ";
    ...
    protected function dependFormulesInit() {
        ...
        $this->formules['calc_B0_Amount'] = function() {
            ...
            $AllNewClubMembers = $this->club_users['B0']['countNew'] +
                    $this->club_users['B1']['countNew'] +
                    $this->club_users['B2']['countNew'] +
                    $this->club_users['B3']['countNew'];
            $AllClubMembers = $this->club_users['B0']['count'] +
...
//            echo 'Юзеры B1' . PHP_EOL;
//            var_dump($this->club_users['B0']);
//            echo 'Юзеры B2' . PHP_EOL;
//            var_dump($this->club_users['B1']);
//            echo 'Юзеры B3' . PHP_EOL;
//            var_dump($this->club_users['B2']);
