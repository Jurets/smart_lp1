...
<div class="wide form">
    ...
    if ($this->action->id === 'create') {
        echo $form->dropDownListControlGroup($model, 'bot', $model->bot, array(
    ...
    if ($this->action->id === 'update') {
        if ($model->tariff_id < Participant::BOT_50) { // это живой юзер - никаких ботов
            echo $form->dropDownListControlGroup($model, 'bot', [], array(
                'displaySize' => '1', 'class' => 'span3',
                'prompt' => BaseModule::t('rec', '--'),
                'options' => array(
                    $model->tariff_id => array(
        ...
        } else if ($model->tariff_id >= Participant::BOT_50 && $model->tariff_id <= Participant::BOT_BC_GOLD) { // тут уже боты, unset задает необратимость изменения статусов
            // add any bot statuses - here
            if ($model->tariff_id == Participant::BOT_BC_BRONZE) {
                unset($model->bot[Participant::BOT_50]);
            }
            /* никакого prompt - только повышение до B0 предвидится */
            echo $form->dropDownListControlGroup($model, 'bot', $model->bot, array(
                'displaySize' => '1', 'class' => 'span3',
