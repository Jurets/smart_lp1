...
abstract class YiiChatDbHandlerBase extends CComponent implements IYiiChat {
	...
	public function yiichat_post($chat_id, $identity, $message, $data){
        ...
        if ($_POST['is_alert'] == 'true' || $_POST['is_alert'] == '1') {
            $is_alert = 1;
        } else {
            $is_alert = 0;
        } //DebugBreak();
		if($message_filtered != ""){
			$obj = array(
				...
				"owner"=>substr($this->getIdentityName(),0,255),
				"created"=>time(),
            ...
            //выборка флага и аватарки юзера
            $user = $this->getDb()->createCommand()
                ->select('users.photo, countries.code')
                ->from('{{users}} users')
                ->leftJoin('cities', 'cities.id = users.city_id')
                ->leftJoin('countries', 'countries.id = cities.country_id')
                ->where('users.id = :id')
                ->queryRow(true, array(':id'=>$identity));
            $obj['photo'] = Participant::buildUrlAvatar($user['photo']);
            $obj['code'] = $user['code'];
			return $obj;
		}
		else
	 ...
	 */
	public function yiichat_list_posts($chat_id, $identity, $last_id, $data, $mode=1){
		$this->_chat_id = $chat_id;
		$this->_identity = $identity;
		$this->_data = $data;
		$limit = 3;
        ...
        if($last_id == -1) 
        {
            $where_string = $this->getTableName().'.chat_id=:chat_id';
            //построить WHERE выражение для выборки по периоду
            $where_string = $this->buildPeriodCondition($where_string, $mode);  
            
            $where_params = array(
                ':chat_id' => $chat_id,
            );
            $rows = $this->db->createCommand()
            ->select($this->getTableName().'.*, users.photo, countries.code')
            ->from($this->getTableName())
            ->leftJoin('{{users}} users', $this->getTableName().'.post_identity = users.id')
            ->leftJoin('cities', 'cities.id = users.city_id')
            ->leftJoin('countries', 'countries.id = cities.country_id')
            ->where($where_string, $where_params)
            //->limit(1)
            ->order($this->getTableName().'.created asc')
            ->queryAll(); 
            foreach($rows as $k=>$v) {
            ...
            $where_string = '(('.$this->getTableName().'.chat_id=:chat_id) and ('.$this->getTableName().'.post_identity<>:identity))';
            $where_string = $this->buildPeriodCondition($where_string, $mode);
            $where_params = array(
                ':chat_id' => $chat_id,
                ':identity' => $identity,
            ...
            ->leftJoin('countries', 'countries.id = cities.country_id')
            ->where($where_string,$where_params)
            ->order($this->getTableName().'.created desc') // in this case desc,late will be sort asc 
            ->queryAll();
    ...
    * 
    */
    private function buildPeriodCondition($where_string, $mode = 1) {
        switch ($mode) {
            case 1: $where_add = $this->getTableName().'.created >= unix_timestamp(DATE_ADD(DATE(NOW()), INTERVAL -1 DAY))'; break;
            case 2: $where_add = $this->getTableName().'.created >= unix_timestamp(DATE_ADD(DATE(NOW()), INTERVAL -7 DAY))'; break;
            case 3: $where_add = $this->getTableName().'.created >= unix_timestamp(DATE_ADD(DATE(NOW()), INTERVAL -30 DAY))'; break;
            case 4: $where_add = $this->getTableName().'.created >= unix_timestamp(DATE_ADD(DATE(NOW()), INTERVAL -3 MONTH))'; break;
            case 5: $where_add = $this->getTableName().'.created >= unix_timestamp(DATE_ADD(DATE(NOW()), INTERVAL -6 MONTH))'; break;
            case 6: $where_add = $this->getTableName().'.created >= unix_timestamp(DATE_ADD(DATE(NOW()), INTERVAL -1 YEAR))'; break;
            case 7: $where_add = ''; break;  //режим: с самого начала
        }  
        $arr = array();
        if (!empty($where_string)) $arr[] = $where_string;
        if (!empty($where_add)) $arr[] = $where_add;
        $where_string = implode(' AND ', $arr);
        return $where_string;
    }
